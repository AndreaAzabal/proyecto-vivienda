...
```{r}
tabla.raw[tabla.raw$price==696000 | (tabla.raw$price==365000 & tabla.raw$metros==84 & tabla.raw$baths==2 ) | tabla.raw$price==448000,]
```
...
```
**Se han eliminado 84 de 94, quedan 10 que tienen dirección exacta en todos los casos o en al menos uno de ellos**
...
```
Ordeno al revés y así elimino fijo todos los que tienen exacta = 0:
...
```{r}
#Hospoints<-Descarga_OSM_points(ciudad="Madrid", key='amenity',value = "hospital")
#coords_hosp <- do.call(rbind, st_geometry(Hospoints$osm_points)) %>% 
...
#Anexo TFM: ANALISIS DE VARIABLES


#VARIABLES USADAS EN LOS MODELOS
#log.pm,habitaciones,baths,terraza,ascensor,aire,garaje,piscina,trastero,armarios,reformar,exterior,balcon,duplex,estudio,piso,dens_cc,dens_hospital,log.dist_tp,dens_colegios


```{r warning=FALSE, message=FALSE}
#Defino vector de numericas
numericas_hist<-dplyr::select(tabla,log.pm,habitaciones,baths,terraza,ascensor,aire,garaje,piscina,trastero,armarios,reformar,exterior,balcon,duplex,estudio,piso,dens_cc,dens_hospital,log.dist_tp,dens_colegios)
#Represento
for (i in 1:ncol(numericas_hist)){
pr<-Hist(numericas_hist,response = numericas_hist[,1],predicted = 0,var = numericas_hist[,i],n=i,breaks = 10)
plot(pr)
}
```
...
```{r}
#Vector de double
numericas_outliers<-dplyr::select(tabla,log.pm,habitaciones,baths,terraza,ascensor,aire,garaje,piscina,trastero,armarios,reformar,exterior,balcon,duplex,estudio,piso,dens_cc,dens_hospital,log.dist_tp,dens_colegios)
for (i in names(numericas_outliers)){
boxplot(numericas_outliers[[i]], main=colnames(numericas_outliers[i]), xlab=colnames(numericas_outliers[i]))
}
```
...
```{r}
#Vector de enteros
numericas_int<-dplyr::select(tabla,log.pm,habitaciones,baths,terraza,ascensor,aire,garaje,piscina,trastero,armarios,reformar,exterior,balcon,duplex,estudio,piso,dens_cc,dens_hospital,log.dist_tp,dens_colegios)
...
```{r}
#Vector de double
numericas_outliers<-dplyr::select(tabla,-distrito, -calle)
for (i in names(numericas_outliers)){
boxplot(numericas_outliers[[i]], main=colnames(numericas_outliers[i]), xlab=colnames(numericas_outliers[i]))
}
```
...
```{r}
binarias<-dplyr::select(tabla, terraza, ascensor, aire, exacta, garaje, piscina, zonas.verdes, trastero, armarios, buen.estado, reformar, exterior, balcon, chalet, atico, duplex, estudio, piso)

for (i in names(binarias)){
  print(colnames(binarias[i]))
print(table(binarias[[i]]))
}
...
```{r}
tabla_dummy<-pivot_wider(tabla,
            names_from = "distrito", 
            values_from = 'distrito', 
            values_fill = 0,
            values_fn = function(x) 1)
colnames(tabla_dummy) <- gsub("-", "_", colnames(tabla_dummy))
tabla_dummy
```
...
```{r}
#glimpse(tabla_mod)
#glimpse(tabla_mod[,-c(1,2,3,4,5,8,20,22,25,32,34,37,38),drop=FALSE])
tabla_mod<-tabla[,-c(1,2,3,4,5,8,20,22,25,32,34,37,38),drop=FALSE]
```

```{r, fig.height=16, fig.width=16}
numericas<-dplyr::select(tabla_mod,-calle,-distrito)
cr <- cor(numericas, use="complete.obs")
ggcorrplot(cr, hc.order = TRUE,type = "lower",lab = TRUE)
```
...
```{r}
write.csv(tabla,"tabla_raw.csv", row.names = FALSE)
write.csv(tabla_dummy,"tabla_dummy.csv", row.names = FALSE)
#write.csv(tabla_mod,"tabla.csv", row.names = FALSE)
```
